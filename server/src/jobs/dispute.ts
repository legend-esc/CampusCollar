...
const worker = new Worker('dispute-resolution', async () => {
  ...
  for (const dispute of staleDisputes) {
    ...
    await notificationService.notify(job.customerId, 'Dispute Resolved', `Your dispute for job "${job.title}" has been auto-resolved. Escrow split 50/50.`)
    if (job.workerId) {
      await notificationService.notify(job.workerId, 'Dispute Resolved', `Your dispute for job "${job.title}" has been auto-resolved. Escrow split 50/50.`)
    }
