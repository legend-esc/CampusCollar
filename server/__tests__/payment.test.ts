...
describe('Payment service', () => {
  beforeAll(async () => {
    const customer = await prisma.user.create({
      data: { email: `pay_c_${Date.now()}@campus.edu`, passwordHash: 'x', name: 'C', university: 'U', emailVerified: true },
    });
    const worker = await prisma.user.create({
      data: { email: `pay_w_${Date.now()}@campus.edu`, passwordHash: 'x', name: 'W', university: 'U', emailVerified: true },
    });
    customerId = customer.id;
    workerId = worker.id;

    const job = await prisma.job.create({
      data: { title: 'Pay test', description: 'x', amount: 100, customerId, workerId, status: 'COMPLETED' },
    });
    jobId = job.id;
